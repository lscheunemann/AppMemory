...
<head>
    ...
    if (isset($id) && $id > 0) {
        ...
        $idEnteNovo = mysqli_insert_id($conn);

        // Segundo insert usando o $id recém-criado
        mysqli_query($conn, "INSERT INTO tb_configuracoes (
            cliente, parceiro, ente, qrcode_ente, permite_curtir, permite_depoimentos, ativo
        ) VALUES (
            '$cliente', '$parceiro', '$idEnteNovo', 'em construção', 0, 0, 0
        )") or die("Não foi possível cadastrar configuração do ente!");
    }
